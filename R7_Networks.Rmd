...
We can use the link to read this CSV file:
...
The adjacency matrix needs not include the  column ("Multinacional") in the data frame:
...
There is some information above you need not pay attention.
...
```{r, eval=FALSE}
EliteNet=set_vertex_attr(EliteNet,"multi",value=EdgesAsDF$Multinacional)
```
...
```
* Shorter path (average): it gets the average of every shortest path among the nodes in the network. A shorter path is the shortest _walk_ fromone node to another.
